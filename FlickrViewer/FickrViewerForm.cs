...
namespace FlickrViewer
{
   public partial class FickrViewerForm : Form
   {
        ...
        private const string KEY = "bf08c4a10f3f786902b042e28a300aef";
         ...
         object sender, EventArgs e)
      {
         if (imagesListBox.SelectedItem != null)
         {
            ...
            string imageFilename = ((FlickrResult)imagesListBox.SelectedItem).Title;
            ...
            if (imagesListBox.SelectedItem != null && imagesListBox.SelectedIndex != -1)
            {
                string selectedURL = ((FlickrResult)imagesListBox.SelectedItem).URL;
                string imageFilename = ((FlickrResult)imagesListBox.SelectedItem).Title;
                ...
                using (var memoryStream = new MemoryStream(imageBytes))
                {
                    Parallel.Invoke(() => pictureBox.Image = Image.FromStream(memoryStream), () => SaveImageLocal(imageBytes, imageFilename), () => CreateSaveThumbNail(imageBytes, 75, imageFilename));
                }

            }
        }
        public void SaveImageLocal(byte[] imagesBytes, string imageFileName) 
      {
            using (var stream = new MemoryStream(imagesBytes))
            {
                var image = Image.FromStream(stream);
                image.Save(imageFileName+".Jpeg", ImageFormat.Jpeg);
              
            }
      }
      public void CreateSaveThumbNail(byte[] imagesBytes, int width, string imageFileName) 
      {
            using (var stream = new MemoryStream(imagesBytes)) 
            {
                var image = Image.FromStream(stream);
                ...
                using (var thumbnailStream = new MemoryStream()) 
                {
                    ...
                    File.WriteAllBytes(imageFileName+"_thumbnail.Jpeg", thumbnailStream.ToArray());
                }
            } 
      }
